...
class ControlBurnClassifier:
    ...
    def __init__(self,alpha = 0.1,max_depth = 10, build_forest_method = 'bagboost', random_state = None,
        ...
        if polish_method.random_state is None:
            polish_method.random_state = random_state
    ...
    def bag_forest(self,X,y):
        ...
        if self.random_state is None:
            _locaL_random_seed = 0
        else:
            _locaL_random_seed = self.random_state
        ...
        for depth in range (1,max_depth+1):
            ...
            while converged == False:
                train1 = train.sample(n = len(train), replace = True, random_state = _locaL_random_seed)
                y1 = train1['y']
                X1 = train1[features]
                clf = DecisionTreeClassifier(max_depth = depth, random_state = _locaL_random_seed)
                clf.fit(X1,y1)
    ...
    def bagboost_forest(self,X,y):
        ...
        while OOB_converged == False:
            ...
            while converged == False:
                ...
                y1 = train1['y']
                X1 = train1[features]
                tree = DecisionTreeRegressor(max_depth = depth, random_state = _locaL_random_seed)
                tree.fit(X1,y1)
    ...
    def solve_lasso(self, costs = [], groups = [], sketching = 1.0):
        ...
        if sketching < 1.0:
            ...
            to_sample = X.copy()
            to_sample['y'] = y
            to_sample = to_sample.sample(n = nsample, random_state = self.random_state)
            y = to_sample['y']
...
#initializer
    ...
    def bag_forest(self,X,y):
        ...
        for depth in range (1,max_depth+1):
            ...
            while converged == False:
                ...
                y1 = train1['y']
                X1 = train1[features]
                regressor = DecisionTreeRegressor(max_depth = depth, random_state = _locaL_random_seed)
                regressor.fit(X1,y1)
    ...
    def solve_lasso(self , costs = [], groups = [], sketching = 1.0):
        ...
        fit = Lasso(alpha = alpha,fit_intercept= False, positive = True, random_state = self.random_state).fit(transformed_matix,y)
